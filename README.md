...
Filter by Maintenance Records (Feature 1)
...
Vehicle Listing
The Vehicle Listing module serves as the primary data aggregator for the application. It maps an individual vehicle profile to an active marketplace post, tracking the listing price, current lifecycle status ("Pending", "Active", "Reserved", "Sold"), and booking criteria.
...
Hatchback Ads
The Hatchback Ads component covers economic and compact urban vehicles. It tracks unique space-saving design attributes such as foldable rear seats and boot mechanisms.
...
Bike Ads
The Bike Ads section displays two-wheeler listings. It manages specific motorbike categories (Sports, Commuter, Cruiser) and details like engine displacement (CC) and electric start availability.
...
Buyer Accounts
The Buyer Accounts module holds client profiles, liquid cash wallet balances, and saved vehicle watchlists. It contains user-driven interface logic to directly purchase token securities on active ad posts.
...
Seller Accounts
The Seller Accounts structure manages vendor data tiers ("Individual" or "Dealer") and holds references to all vehicle listings published by that specific seller profile.
...
Admin Compliance Controls
The Admin class provides administrative control over marketplace inventory. It uses Friend Class permissions to directly modify an advertisement's protected lifecycle state variables for approvals or safety removals.
...
Filter by Brand
This search function allows buyers to find vehicles by their manufacturer brand name, saving users from scrolling through all public listings.
...
Filter by Maintenance Records (Feature 1)
This feature isolates listings based on their recorded maintenance upkeep logs. Buyers can screen for vehicles that meet or exceed a specific threshold of documented service records.
...
View Reserved Bookings (Feature 2)
This module handles transactional bookings. It parses the entire live data array and displays only the vehicle assets currently locked under a secure user token money placement.
...
Compare Listings & App Statistics
This function handles backend platform audits and comparisons. It utilizes Operator Overloading (<, ==, and +) to perform clean comparisons of asset prices and aggregate marketplace data blocks.
